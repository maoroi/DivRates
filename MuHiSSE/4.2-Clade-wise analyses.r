...
## NEED TO ADD: all terrestrial mammals (no bats and marine mammals), 
##              Euarchontoglires, 
##              Laurasiatheria, 
##              Afrotheria ?
##              Rodentia clades - Muroidea, Castorimorpha, squirrels and kin; depending on AP repesentation
...
#}   
    ...
    for (n in 1:5){
        ...
        sink(file=f)
            ...
            # variable line
            cat(paste0("/usr/bin/time --verbose R --no-save < /lustre/scratch/scratch/ucbtmao/MuHiSSE/VR/",ord,"/", ord,"_vrMuHiSSE",n+1,"_MCC_Code.R> ",ord,"_vrMuHiSSE",n+1,"_MCC_Code.out"), sep="\n")
            cat("", sep="\n")
...
## CID Shell file
        ...
        sink(file=f)
            ...
            cat("# state program running time", sep="\n")
            # variable line
            cat(paste0("/usr/bin/time --verbose R --no-save < /lustre/scratch/scratch/ucbtmao/CID/VR/",ord,"/", ord,"_vrCID",n+1,"_MCC_Code.R> ",ord,"_vrCID",n+1,"_MCC_Code.out"), sep="\n")
            cat("", sep="\n")
            cat("# tar-up (archive) all output files to transfer them back to your space.", sep="\n")
            # variable line
            cat(paste0("tar -zcvf $HOME/Scratch/CID/VR/",ord,"/files_from_job_$JOB_ID.tgz $TMPDIR"), sep="\n")
            cat("", sep="\n")
            # variable line
