...
#  2. Updating to MDD v1.1 ------------------------------------------------
# (MDD data accessed 04/02/2020)

# * 2.1 Taxonomy ----------------------------------------------------------
tax <- read.csv("C:/Users/Roi Maor/Desktop/New Mam Phylo/Burgin taxonomy/MamTax2018.csv", header = TRUE)
...
#  4. Updating to MDD v1.0 ------------------------------------------------
# (MDD data accessed 07/01/2019)

# * 2.1 Taxonomy ----------------------------------------------------------
tax <- read.csv("C:/Users/Roi Maor/Desktop/New Mam Phylo/Burgin taxonomy/asm-species-2019-01-07.csv", header = TRUE)
tax <- tax[-1,c(1,2,4,15,16,19,20)]
tax$Canonical.Sciname <- gsub(' ', '_', tax$Canonical.Sciname, ignore.case = FALSE)

# * 2.2 Retain unchanged binomials ----------------------------------------
unchanged <- which(data$MSW3 %in% as.character(tax$Canonical.Sciname))
data$MDD[unchanged] <- data$MSW3[unchanged]
# consistency test
length(which(is.na(data$MDD))) == 249
